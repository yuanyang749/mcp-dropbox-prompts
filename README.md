...
提示词的生命力在于多样性与实战验证。我们鼓励社区共同维护和丰富 `prompts` 目录，打造更高质量的 AI 角色库：

- **分享你的 Prompt**：如果你有调优成熟的 AI 角色，欢迎提交 PR 到 `/prompts` 目录分享给更多人。
- **持续优化**：提示词需要不断迭代，欢迎对现有 Prompt 进行精度纠偏或功能增强。
- **实战反馈**：在不同模型（如 GPT, Claude 等）下的表现差异，欢迎在 Issues 或讨论区分享。

让我们通过社区的力量，共同构建一套开箱即用、专业级的中英文提示词资产。
